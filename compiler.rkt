...
(define (ifarith->ifarith-tiny e)
  (match e
    ...
    ['false e] ;2
    [(? symbol? x) x] ;3
    [`(,(? bop? bop) ,e0 ,e1) `(,bop ,(ifarith->ifarith-tiny e0) ,(ifarith->ifarith-tiny e1))] ;4
    [`(,(? uop? uop) ,e) `(,uop ,(ifarith->ifarith-tiny e))] ;sec05
    ;; 0-binding case
    [`(let* () ,e) (ifarith->ifarith-tiny e)] ;MihirM204
    ;; 1+-binding case
    [`(let* ([,(? symbol? x0) ,e0]) ,e-body)
     `(let ([,x0 ,(ifarith->ifarith-tiny e0)]) ,(ifarith->ifarith-tiny e-body))] ;MihirM204 / Code2021X
